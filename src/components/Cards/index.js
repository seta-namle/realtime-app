...
const DashBoardCard = ({ dataChart, chartColor, cardTitle, cardDes, cardValue, cardIcon, type }) => {
    if (type === 'activeTasks') {
        return (
            <Card className={styles['card-dashboard-active']}>
                <div className={styles['card-dashboard-content']}>
                    <Title level={1} className={styles['card-title']} >{formatNumber(cardTitle)}</Title>
                    <Text>{cardDes}</Text>
                </div>
            </Card>
        )
                ...
                <Col span={24}>
                    <ResponsiveContainer width="100%" height={85} >
                        <AreaChart data={dataChart}
                            margin={{}}>
                            <Area type='monotone' dataKey='uv' stroke={chartColor} fill={chartColor} />
                        </AreaChart>
                    </ResponsiveContainer>
                </Col>
            </Row>
